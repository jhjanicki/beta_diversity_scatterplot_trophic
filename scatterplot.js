...
// 					  .style("opacity",0.9);
					...
					var frugivore = _data.filter(function(d){
						return d.Trophic == 'Frug' && d.LandSea;
						
					});
					
					var frugivoreDot = _data.filter(function(d){
						return d.Trophic == 'Frug' && d.LandSea == landsea && d[yValue] != null;
					...
					var insectivoreDot = _data.filter(function(d){
						return d.Trophic == 'Invert' && d.LandSea == landsea && d[yValue] != null;
					});
				  ...
				  container.selectAll(".dot2"+_index)
					...
					var omnivoreDot = _data.filter(function(d){
						return d.Trophic == 'Omni' && d.LandSea == landsea && d[yValue] != null;
					});
					...
					var granivoreDot = _data.filter(function(d){
						return d.Trophic == 'Grani' && d.LandSea == landsea && d[yValue] != null;
					});  
					  
					container.selectAll(".dot4"+_index)
					  .data(granivoreDot)
					  .enter().append("path").attr("d", d3.svg.symbol().type("triangle-down"))
					  .attr("class", "dot")
					  .attr("transform", function(d){ return "translate(" + x(d[xValue]) + "," +y(d[yValue]) + ")"})
					  .style("fill", '#0072B2')
					  .style("opacity",0.6);
					...
					//sea
					 var carnivore = _data.filter(function(d){
					  ...
					  var carnivoreDot = _data.filter(function(d){
						return d.Trophic == 'Vert' && d.LandSea == landsea && d[yValue] != null;
					 }); 
